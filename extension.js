...
function getConfig() {
  ...
  return {
    delayTimerDuration: config.get('delayTimer', 500),
    maxHistoryLimit: config.get('maxHistoryLimit', 50)
  };
}
...
function handleLineChange(editor) {
  const { delayTimerDuration } = getConfig();
...
function addToHistory(documentUri, lineNumber, column) {
  const { maxHistoryLimit } = getConfig();

  const { lineHistory, currentIndex } =
    documentLineHistories[documentUri];
